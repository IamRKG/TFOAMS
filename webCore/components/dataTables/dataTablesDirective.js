angular.module('WebCore.Components')
	.directive('datatable', ['$compile', '$window', '_',
		function($compile, $window, _) {
			return {
				restrict: 'A',
				require: ['datatable', '?ngModel'],
				scope: {
					headerTemplate: '@headerTemplate',
					overrideOptions: '@datatable',
					columns: '=columns',
				...
				controller: ['$compile', '$scope', '$element', '$attrs',
					function($compile, $scope, $element, $attrs) {
							...
							if(this.showcheckBoxColumn) { //so that check boxes show even if chosen column is index 0
								var checkBoxCol = {
									...
									mRender: function(data) {
										if(data) {
											return '<label class="sr-only" for="checkbox_' + data + '">Select ' + data + '</label><input type="checkbox" id="checkbox_' + data + '" ' + inputName + ' ' + requiredString + ' class="checkbox table-check-box" value="' + data + '"/>';
										} else {
											return '';
										}
									},
						...
						this.adjustColumns = function() {
							...
							if(this.currentBreakPoint !== this.previousBreakPoint) {
								this.defineTable(_.defaults(options, this.options));
					...
					return {
						...
						post: function($scope, iElement, iAttrs, controllers) {
							...
							if(datatableController.initialize(ngModelController)) {
								...
								iElement.find('.paginator select').addClass('form-control input-sm');
